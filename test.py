...
def evaluation(encoder, bn, decoder, dataloader,device,_class_=None):
    ...
    with torch.no_grad():
        for img, gt, label, _ in dataloader:
            ...
            labels.append(label.numpy().astype(int))
        ...
        ################################################################################################
        # fpr, tpr, thresholds = metrics.roc_curve(gt_list_px, pr_list_px, pos_label=1)
        # print(len(fpr))
        # print(len(tpr))
        # plt.xlabel("fpr")
        # plt.ylabel("tpr")
        ...
        # plt.plot(x, y, label="auroc_px")
        # plt.legend()
        # plt.show()
        _, idx = torch.sort(torch.Tensor(pr_list_sp), descending=False)
        print(idx[:5])
        print(pr_list_sp[idx[:5][0]], pr_list_sp[idx[:5][1]], pr_list_sp[idx[:5][2]], pr_list_sp[idx[:5][3]], pr_list_sp[idx[:5][4]])
        print(labels[idx[:5][0]], labels[idx[:5][1]], labels[idx[:5][2]], labels[idx[:5][3]], labels[idx[:5][4]])
        #############################################
...
def compute_pro(masks: ndarray, amaps: ndarray, num_th: int = 200) -> None:
    ...
    # plt.ylabel("pro")
    # data_dict = {}
    # for i, j in zip(df["fpr"], df["pro"]):
    #     data_dict[i] = j
    # x = [i for i in data_dict.keys()]
    # y = [i for i in data_dict.values()]
    # plt.plot(x, y, label="aupro_px")
    # plt.legend()
    # plt.show()
